...
If any of the below rules need to be broken, raise a action required for the user to resolve.
...
<mcp_assign_usage>
**ALWAYS** use MCP Assign tool in most cases, and only in rare occasions should Handoff be considered. Handoff is blocking, and will cause messages from workers to be blocked until the handoff task is done. 

**Assignments are fire-and-forget.** Once all assignments are delegated, the agent should remain idle and await further instructions. 
</mcp_assign_usage>
